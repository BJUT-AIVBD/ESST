...
from selfconv.Involution import Involution
...
class Attentionv1(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape  #2,16384,64
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) #2,1,16384,64

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_) #2,256,64
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) #2,2,1,256,64
        else:
            kv = self.kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1] #2,1,256,64

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn) #2,1,16384,256

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)#2,16384,64

        return x

#########xiugai self+invo(conv) work2
class Attentionv2(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.involution = Involution(kernel_size=3, in_channel=9 * dim // self.num_heads, stride=1)
        self.pointwise_conv = nn.Conv2d(9 * dim // self.num_heads, dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, H, W):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x) #2,16384,64

        #add
        qkv = self.qkv(x)  # 2,16384,192 #2,133,133,288

        # fully connected layer
        f_all = qkv.reshape(x.shape[0], H * W, 3 * self.num_heads, -1).permute(0, 2, 1, 3)  # # B, 3*nhead, H*W, C//(3*nhead)  f_all(2,9,17689,32) (2,3,16384,64)

        f_conv = self.fc(f_all).permute(0, 3, 1, 2).reshape(x.shape[0], 9 * x.shape[-1] // self.num_heads, H,
                                                            W)  # B, 9*C//nhead, H, W  ##f_conv(2,288,133,133)
        # group conovlution
        # out_conv = self.dep_conv(f_conv).permute(0, 2, 3, 1).reshape(B, N, C)  # B, H, W, C (2,133,133,96)

        # ##Involution
        invo_conv = self.involution(f_conv) #(2,192,128,128)
        out_conv = self.pointwise_conv(invo_conv).permute(0, 2, 3, 1).reshape(B, N, C) #2,128,128,64)
