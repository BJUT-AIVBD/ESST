    ...
    def __init__(self, proj_type='pool', patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        if proj_type == 'conv':
            self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size,
                                  groups=patch_size[0] * patch_size[1])
        elif proj_type == 'pool':
...
class LargewindowHead(BaseDecodeHead):
    def __init__(self, embed_dim=768, use_scale=True, reduction=2, **kwargs):
        super(LargewindowHead, self).__init__(
            input_transform='multiple_select', **kwargs)

        ##XIUGAI
        self.lawin_ms_8 = LawinAttn(in_channels=512, reduction=reduction, use_scale=use_scale, conv_cfg=self.conv_cfg,
                                 norm_cfg=self.norm_cfg, mode='embedded_gaussian', head=64, patch_size=8)
        self.lawin_os_8 = LawinAttn(in_channels=512, reduction=reduction, use_scale=use_scale, conv_cfg=self.conv_cfg,
                                 norm_cfg=self.norm_cfg, mode='embedded_gaussian', head=64, patch_size=8)
        ...
        ####test no_c1(因为要测试没有c1添加的low_level_no)
        # self.output_cat_noc1 = ConvModule(
        #     in_channels=512 * 3,
        #     # in_channels=512 * 4,
        #     out_channels=512,
        #     kernel_size=1,
        ...
        ##XIUGAI
        self.ds_1 = PatchEmbed(proj_type='conv', patch_size=1, in_chans=512, embed_dim=512, norm_layer=nn.LayerNorm)

    def get_context(self, x, patch_size):
        n, _, h, w = x.shape
        context = []
        for i, r in enumerate([8, 4, 2, 1]):
    ...
    def forward(self, inputs):
        ...
        context_ms = self.context_cat(torch.cat(context[:-1], dim=1))
        context_os = context[-1]
        ...
        _output_ms = self.lawin_ms_8(query, context_ms)
        _output_ms = rearrange(_output_ms, '(b nh nw) c ph pw -> b c (nh ph) (nw pw)', ph=patch_size, pw=patch_size,
                            nh=h // patch_size, nw=w // patch_size)
        output.append(_output_ms)
        _output_os = self.lawin_os_8(query, context_os)
        _output_os = rearrange(_output_os, '(b nh nw) c ph pw -> b c (nh ph) (nw pw)', ph=patch_size, pw=patch_size,
                               nh=h // patch_size, nw=w // patch_size)
        output.append(_output_os)

        # xiugi
        output = self.output_cat(torch.cat(output, dim=1))
       ...
       ##xiugai no_c1
        ...
        #add
        # output = self.output_cat_noc1(torch.cat(output, dim=1))
